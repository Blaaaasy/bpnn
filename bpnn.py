...
def demo():
    ...
    # 读取CSV文件
    df = pd.read_csv('Tianchi_Train_uid_iid_rt.csv')
    pat = [[[user_id, click_article_id], [rating]] for user_id, click_article_id,
           rating in zip(df['user_id'], df['click_article_id'], df['ratings'])]
    ...
    df2 = pd.read_csv('Tianchi_Train_uid_iid.csv')
    pat2 = [[[user_id, click_article_id], [rating]] for user_id, click_article_id,
            rating in zip(df2['user_id'], df2['click_article_id'], df2['ratings'])]
    n.test(pat2)
